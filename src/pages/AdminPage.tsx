...
type ModerationTabProps = {
  pendingClubs?: PendingClubRow[];
  pendingPromos?: PromotionRow[];
  handleApprove: (id: string, name: string) => void;
  handleReject: (id: string, name: string) => void;
  handleApprovePromo: (id: string) => void;
  handleRejectPromo: (id: string) => void;
  approveClub: { isPending: boolean };
  rejectClub: { isPending: boolean };
};
...
const StatCard = ({
  ...
  <div className="glass rounded-2xl p-4 border border-border/40 relative overflow-hidden">
    <div
      className={`absolute -top-8 -right-8 w-24 h-24 rounded-full blur-2xl opacity-30 bg-${tone}`}
    />
...
const EmptyState = ({ icon: Icon, title, sub }: EmptyStateProps) => (
...
const OverviewTab = () => {
  ...
  const removeItem = async (kind: string, id: string, label: string) => {
    ...
    try {
      await del.mutateAsync({ table, id, label });
  ...
  const runSync = async () => {
    try {
      ...
      const result = await sync.mutateAsync("refresh");
      toast.success(`Synced ${result?.count ?? 0} items`);
    } catch (error: unknown) {
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <ActionCard
            icon={Music}
            label="Pending spots"
            count={s?.pendingClubs}
            tone="primary"
          />
          <ActionCard
            icon={Megaphone}
            label="Pending promos"
            count={s?.pendingPromo}
            tone="destructive"
          />
        </div>
      </div>
      ...
      <div>
        <h2 className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-3 flex items-center gap-2">
          ...
          <Button
            ...
            className="ml-auto h-7 gap-1.5 text-[10px]"
          >
            <RefreshCw
          ...
          {!feed?.length ? (
            ...
            feed.map((e: ActivityEvent, i) => (
              <div key={i} className="px-3 py-2 flex items-center gap-3 group">
                ...
                {ACTIVITY_TABLE_MAP[e.kind ?? ""] && (
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() =>
                      removeItem(e.kind ?? "", e.id ?? "", e.label)
                    }
...
const ChatModerationPanel = () => {
  ...
  const remove = async (id: string) => {
    ...
    try {
      await del.mutateAsync({ table: "messages", id, label: "chat message" });
            ...
            <div key={m.id} className="px-3 py-2 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground">
                  ...
                  {m.is_hidden ? " · hidden" : ""}
                  {m.flag_count > 0 ? ` · ${m.flag_count} flags` : ""}
                </p>
              </div>
              <Button
                size="icon"
...
const ActionCard = ({
  ...
  <div className="glass rounded-2xl p-4 border border-border/40 flex items-center gap-3">
    <div
      className={`w-10 h-10 rounded-xl bg-${tone}/15 text-${tone} flex items-center justify-center`}
    >
  ...
  return (
    <div className="space-y-4">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isMod && (
                      <Badge className="bg-secondary/20 text-secondary text-[9px]">
                        MOD
                      </Badge>
                    )}
                    {u.is_banned && (
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Joined {lastActive} · {u.points} pts
                  </p>
                </div>
      ...
      <p className="text-[10px] text-muted-foreground text-center">
        Tap any user to manage profile, roles, warnings, blocks &amp; bans.
      </p>
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto glass border-border/40">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Settings2 className="w-4 h-4 text-primary" /> Manage{" "}
            {user.username || "user"}
          </DialogTitle>
          <DialogDescription className="text-[11px] font-mono opacity-60">
            {user.user_id}
          </DialogDescription>
        </DialogHeader>
        ...
        <section className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Button
              size="sm"
              onClick={() =>
                wrap("Profile updated", () =>
                  actions.updateProfile.mutateAsync({
                    userId: user.user_id,
                    patch: {
        ...
        <section className="space-y-2">
          ...
          <div className="flex items-center justify-between glass rounded-lg p-3">
            ...
            <Switch
              checked={!!isAdmin}
              onCheckedChange={(v) =>
                wrap(v ? "Admin granted" : "Admin revoked", () =>
                  actions.setRole.mutateAsync({
                    userId: user.user_id,
                    role: "admin",
                    grant: v,
                  }),
          ...
          <div className="flex items-center justify-between glass rounded-lg p-3">
            ...
            <Switch
              checked={!!isMod}
              onCheckedChange={(v) =>
                wrap(v ? "Moderator granted" : "Moderator revoked", () =>
                  actions.setRole.mutateAsync({
                    userId: user.user_id,
                    role: "moderator",
                    grant: v,
        ...
        <section className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={() =>
                wrap("Points set", () =>
                  actions.adjustPoints.mutateAsync({
                    userId: user.user_id,
                    points,
                  }),
                )
        ...
        <section className="space-y-2">
          <h3 className="text-xs uppercase tracking-wider text-muted-foreground font-bold flex items-center gap-1.5">
            <AlertOctagon className="w-3.5 h-3.5" /> Enforcement
          </h3>
          <Textarea
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button
              ...
              onClick={() =>
                wrap("Warning issued", () =>
                  actions.warn.mutateAsync({ userId: user.user_id, reason }),
                )
              }
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                wrap(user.is_blocked ? "Unblocked" : "Blocked", () =>
                  actions.setBlocked.mutateAsync({
                    userId: user.user_id,
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                wrap(user.is_banned ? "Unbanned" : "Banned", () =>
                  actions.setBanned.mutateAsync({
                    userId: user.user_id,
              ...
              className="border-destructive/40 text-destructive col-span-2"
            >
              <Ban className="w-3.5 h-3.5 mr-1" />{" "}
              {user.is_banned ? "Lift ban" : "Ban account"}
            </Button>
          </div>
        </section>
...
const CheckinsTab = () => {
  ...
  const exportCsv = () => {
    ...
    const body = data.rows
      .map(
        (r: CheckinRow) =>
          `${r.kind},${String(r.target)},${String(r.device_id ?? "")},${r.created_at}`,
      )
      .join("\n");
    ...
    <div className="space-y-4">
      ...
      {!!data?.dupes?.length && (
        <div className="glass rounded-2xl p-3 border border-destructive/30 bg-destructive/5">
          <p className="text-xs font-bold text-destructive flex items-center gap-1.5 mb-2">
            <ShieldAlert className="w-3.5 h-3.5" /> Fraud alert:{" "}
            {data.dupes.length} duplicate event(s) inside 2 minutes
          </p>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {data.dupes.slice(0, 20).map((d: CheckinRow) => (
              <div
                key={d.id ?? `${d.kind}-${d.created_at}`}
                className="text-[10px] font-mono text-muted-foreground"
              >
                {d.kind} · {String(d.target).slice(0, 8)} · device{" "}
                {String(d.device_id).slice(0, 8)}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="glass rounded-2xl divide-y divide-border/30 max-h-[60vh] overflow-y-auto">
        {isLoading ? (
          ...
          <EmptyState
            icon={UserCheck}
            title="No check-ins in the last 2 hours"
          />
        ) : (
          data.rows.map((r: CheckinRow) => (
            <div
              key={`${r.kind}-${r.id ?? r.created_at}`}
  ...
  <div className="space-y-6">
    ...
    <section>
      <h2 className="font-display font-semibold text-base text-foreground mb-3 flex items-center gap-2">
        <Megaphone className="w-4 h-4 text-secondary" /> Boosts (
        {pendingPromos?.length || 0})
      </h2>
      {!pendingPromos?.length ? (
        <EmptyState icon={Megaphone} title="No pending boosts" />
      ) : (
        <div className="space-y-2">
          {pendingPromos.map((promo: PromotionRow) => (
            <div
              key={promo.id}
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <Badge className="gradient-secondary text-[9px]">
                    {promo.type}
                  </Badge>
                  <span className="text-xs truncate">
                    {promo.profile?.username || "User"}
                  </span>
                </div>
                <p className="text-[10px] text-muted-foreground">
                  Ref <span className="font-mono">{promo.bank_reference}</span>{" "}
                  · R{((promo.amount_cents || 0) / 100).toFixed(2)}
                </p>
              </div>
              <div className="flex gap-1.5 shrink-0">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleRejectPromo(promo.id)}
                  className="h-8 px-2 border-destructive/30 text-destructive"
                >
    ...
    <section>
      <h2 className="font-display font-semibold text-base text-foreground mb-3 flex items-center gap-2">
        <MapPin className="w-4 h-4 text-primary" /> Spots (
        {pendingClubs?.length || 0})
      </h2>
      {!pendingClubs?.length ? (
        <EmptyState icon={Music} title="All caught up" />
      ) : (
        <div className="space-y-2">
          {pendingClubs.map((c: PendingClubRow) => (
            <div
              key={c.id}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-4">
          ...
          <div className="space-y-2.5">
            {topGenres.map(([k, v]) => (
              <Bar key={k} label={k} value={v} max={maxG} />
            )) || null}
          </div>
        </div>
        <div className="glass rounded-2xl p-4">
          ...
          <div className="space-y-2.5">
            {topAreas.map(([k, v]) => (
              <Bar key={k} label={k} value={v} max={maxA} />
            ))}
          </div>
        </div>
      </div>
...
const SystemTab = () => {
  const items = [
    ...
    { label: "Storage", status: "operational", desc: "Buckets reachable" },
    {
      label: "Notifications",
      status: "operational",
      desc: "Realtime delivery active",
    },
    {
      label: "Background jobs",
      status: "operational",
      desc: "Triggers firing",
    },
  ];
  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        {items.map((i) => (
          ...
          >
            ...
            <Badge className="bg-emerald-500/15 text-emerald-400 text-[9px] uppercase">
              {i.status}
            </Badge>
          </div>
        ))}
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <ul className="space-y-1.5 text-xs text-muted-foreground">
          <li>
            · Failed-login monitoring and admin audit logs are streamed from
            SCENE Command Infrastructure.
          </li>
          <li>
            · Role &amp; permission management: edit via the{" "}
            <code className="text-primary">user_roles</code> table.
          </li>
          <li>
            · Two-factor authentication can be enabled per-user in account
            settings.
          </li>
        </ul>
      </div>
...
const AdminPage = () => {
  ...
  const [unlocked, setUnlocked] = useState(
    () => sessionStorage.getItem("scene_admin_unlocked") === "1",
  );
  ...
  const handleApprovePromo = async (id: string) => {
    try {
      await approvePromo.mutateAsync({ id });
  ...
  const handleRejectPromo = async (id: string) => {
    try {
      await rejectPromo.mutateAsync({ id });
      ...
      <main className="container mx-auto px-4 pt-24 pb-12 max-w-5xl">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 rounded-xl bg-primary/20 border border-primary/30 shadow-[0_0_20px_hsl(var(--primary)/0.3)]">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            >
              <Link to="/admin/sponsorships">
                <Sparkles className="w-3.5 h-3.5 mr-1.5" /> Sponsorships
              </Link>
            </Button>
            <Button
              asChild
            ...
            >
              <Link to="/business/billing">
                <Building2 className="w-3.5 h-3.5 mr-1.5" /> Venue Billing
              </Link>
            </Button>
          </div>
          ...
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="w-full overflow-x-auto flex justify-start gap-1 h-auto bg-card/40 p-1 rounded-full">
              ...
              <TabsTrigger
                ...
                className="rounded-full text-xs gap-1.5"
              >
                <Building2 className="w-3.5 h-3.5" /> Venue Claims
              </TabsTrigger>
              <TabsTrigger
                value="analytics"
                className="rounded-full text-xs gap-1.5"
